            ...
            <div class="col-md-10 house-info">
                <div class="content-box-large" style="padding:10px; padding-left:40px; margin-bottom:15px;">
                    <div class="panel-body">
                        <h4 class="title"><i style="margin-right:10px;" class="glyphicon glyphicon-tent"></i> 펜션 등록</h4>
                    </div>
                </div>
            </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="container-fluid tdContainer" style="display:flex">
                                            <textarea class="form-control" id="note" name="note" rows="3" style="font-size:13px;"></textarea>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                ...
                <div class="content-box-large" style="padding-top:20px; padding-bottom:15px;">
                    <div class="panel-body">
                        ...
                        <div class="container-fluid" style="padding:0;">
                            <textarea class="form-control" id="direction" name="direction" rows="12" style="font-size:13px;"></textarea>
                        </div>
                        ...
                        <div class="container-fluid" style="padding:0; margin-left: 100px;">
                            <?php
                            $sql = "SELECT tid, name FROM tourist_spot";
                            $result = mysqli_query($conn, $sql);
                            while($row = mysqli_fetch_assoc($result)) {
                              echo '<label class="radio-inline col-xs-6 col-sm-3"><input type="checkbox" name="chk_info" value="'.$row[tid].'">'.$row[name].'</label>';
                            }
                            ?>
                        </div>
                    </div>
    ...
    <div class="col-md-offset-2 col-md-10 house-info">
        <div class="content-box-large" style="padding:10px; margin-top:15px;">
            <div class="panel-body">
              ...
              <button type="button" id="register5" style="float: right; display:none;" class="col-xs-6 btn btn-success" onclick="submit()"><span class="glyphicon glyphicon-ok" aria-hidden="true">&emsp;등록</span></button>
    ...
    function addressConvert(addr) {
      ...
      geocoder.addr2coord(addr, function(status, result) {
          ...
          if (status === daum.maps.services.Status.OK) {
              ...
              var pid = -1;
              $.ajax({
                url:'./pension_list_upload.php?lat='+result.addr[0].lat+"&lng="+result.addr[0].lng,
                type:'post',
                async:false,
                data:formData,
                success:function(data){
...
<script>
    function submit() {
      ...
      if($("#region").val() != '' && $("#pname").val() != '' && $("#sample6_address").val() != '' && $("#qnumber").val() != '' && $("#checkin").val() != '' && $("#checkout").val() != '' && $("#summber").val() != '' && $("#winter").val() != '' && $("#note").val() != '' && $("#list").children("ul").children("li").children("strong").text() != '') {
